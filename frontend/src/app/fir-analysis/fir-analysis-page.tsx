...
function FIRAnalysisContent() {
  ...
  const handleGenerateBail = async (bailType: 'pre_arrest' | 'post_arrest') => {
    ...
    try {
      // If deep (senior advocate) analysis has already been run for this FIR,
      // ground the bail application in that full issue-by-issue reasoning
      // instead of just the quick single-shot analysis.
      const { data } = await api.post(`/analysis/fir/${analysisId}/bail`, {
        bailType,
        deepAnalysis: deepResult || null,
      });
      setBailDraft(data.data.content);
      toast.success(deepResult ? 'Bail application drafted (senior advocate mode)!' : 'Bail application drafted!');
    } catch (err: any) {
                ...
                <Card>
                  ...
                  {deepResult ? (
                    <p className="text-xs text-primary-700 dark:text-primary-400 flex items-center gap-1.5">
                      <Brain className="w-3.5 h-3.5" /> Will be drafted using the senior advocate deep analysis above.
                    </p>
                  ) : (
                    <p className="text-xs text-slate-400">
                      Tip: run Deep Analysis first so this draft is grounded in full issue-by-issue reasoning, not just the quick summary.
                    </p>
                  )}
