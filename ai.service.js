...
async function generateContent({ contents, systemInstruction, jsonMode = false, maxTokens = 4096, groundingQuery }) {
  try {
    ...
    // Gemini's free tier enforces a fairly low requests-per-minute quota.
    // A 429 (rate limit) or 503 (model overloaded) is almost always
    // transient, so retry once after a short backoff instead of
    // immediately surfacing a 500 to the user for something that would
    // likely succeed a couple of seconds later.
    ...
    for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
      ...
      if (isRetryable && attempt < MAX_ATTEMPTS) {
        ...
        logger.warn(`Gemini API returned ${res.status} (attempt ${attempt}/${MAX_ATTEMPTS}) — retrying in ${retryDelayMs}ms.`);
    ...
    if (!res.ok) {
      const apiMessage = data?.error?.message || `Gemini API returned HTTP ${res.status}`;
      const friendlyMessage = res.status === 429
        ? 'The AI service is currently rate-limited (too many requests in a short time). Please wait a minute and try again.'
        : res.status === 503
    ...
    // The model can come back with zero candidates entirely (e.g. the
    // prompt itself was blocked by safety filters) — data.candidates would
    // be undefined/empty and promptFeedback carries the reason. Surface
    // that clearly instead of silently returning an empty response.
    if (!candidate) {
      const blockReason = data?.promptFeedback?.blockReason;
      if (blockReason) {
        throw new Error(`The AI declined to respond to this request (reason: ${blockReason}). Try rephrasing.`);
      }
      throw new Error('The AI service returned no response candidates.');
    }
    ...
    // If generation stopped because of the output-length cap and we ended
    // up with no usable text at all, that's worth distinguishing from a
    // generic empty response too.
    if (!text && candidate.finishReason === 'MAX_TOKENS') {
      throw new Error('The AI response was cut off before it produced any content (output token limit reached).');
    }
